...
import SandboxSettingsCommand from '../../../src/commands/sandbox/settings.js';
import {
...
describe('sandbox settings', () => {
  ...
  async function setupCommand(flags: Record<string, unknown>, args: Record<string, unknown>): Promise<any> {
    ...
    const command = await createTestCommand(SandboxSettingsCommand as any, config, flags, args);
    (command as any).log = () => {};
    makeCommandThrowOnError(command);
    return command;
  }

  it('calls /sandboxes/{sandboxId}/settings with resolved sandbox id', async () => {
    ...
    stubOdsClient(command, {
      async GET(url: string, options: any) {
        ...
        return {
          data: {
            data: {
              ocapi: [{client_id: 'client-a', resources: []}],
              webdav: [{client_id: 'client-a', permissions: []}],
            },
          },
        };
      },
    ...
    expect(requestUrl).to.equal('/sandboxes/{sandboxId}/settings');
