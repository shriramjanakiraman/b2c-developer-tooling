...
import EcdnPageShieldPoliciesDelete from '../../../../../src/commands/ecdn/page-shield/policies/delete.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../../helpers/test-setup.js';

describe('ecdn page-shield policies delete', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnPageShieldPoliciesDelete, hooks.getConfig(), flags, {});
  }
  ...
  it('deletes policy in JSON mode', async () => {
    ...
    expect(result.policyId).to.equal('policy-1');
  });

  it('deletes policy in non-JSON mode', async () => {
    ...
    const result = (await runSilent(() => command.run())) as {deleted: boolean; policyId: string};
    expect(result.deleted).to.equal(true);
    expect(result.policyId).to.equal('policy-2');
  });
