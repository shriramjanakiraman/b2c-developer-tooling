...
import EcdnMrtRulesGet from '../../../../src/commands/ecdn/mrt-rules/get.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../helpers/test-setup.js';

describe('ecdn mrt-rules get', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnMrtRulesGet, hooks.getConfig(), flags, {});
  }
  ...
  it('returns MRT ruleset in JSON mode', async () => {
  ...
  it('displays ruleset in non-JSON mode', async () => {
    const command: any = await createCommand({'tenant-id': 'zzxy_prd', zone: 'my-zone'});
    ...
    const result = (await runSilent(() => command.run())) as {ruleset: {id: string}};
