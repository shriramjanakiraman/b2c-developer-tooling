...
import EcdnMrtRulesDelete from '../../../../src/commands/ecdn/mrt-rules/delete.js';
import {createIsolatedConfigHooks, createTestCommand} from '../../../helpers/test-setup.js';

describe('ecdn mrt-rules delete', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnMrtRulesDelete, hooks.getConfig(), flags, {});
  }
  ...
  it('deletes MRT ruleset in JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'ruleset-id': 'ruleset-123',
    });
    ...
    expect(result).to.have.property('rulesetId', 'ruleset-123');
  });

  it('deletes ruleset in non-JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'ruleset-id': 'ruleset-456',
    });
    ...
    expect(result.rulesetId).to.equal('ruleset-456');
  });
  ...
  it('errors on API failure', async () => {
    ...
    stubCdnClient(command, {
      DELETE: async () => ({
        data: undefined,
        error: {title: 'Not Found', detail: 'Ruleset not found'},
      }),
    });
