...
import EcdnLogpushJobsGet from '../../../../../src/commands/ecdn/logpush/jobs/get.js';
import {createIsolatedConfigHooks, createTestCommand} from '../../../../helpers/test-setup.js';

describe('ecdn logpush jobs get', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnLogpushJobsGet, hooks.getConfig(), flags, {});
  }
  ...
  it('returns logpush job in JSON mode', async () => {
    ...
    expect(result.job.logFields).to.deep.equal(['ClientIP', 'RayID']);
  });

  it('displays job details in non-JSON mode', async () => {
    const command: any = await createCommand({'tenant-id': 'zzxy_prd', zone: 'my-zone', 'job-id': 123});
