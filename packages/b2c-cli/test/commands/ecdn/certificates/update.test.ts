...
import EcdnCertificatesUpdate from '../../../../src/commands/ecdn/certificates/update.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../helpers/test-setup.js';

describe('ecdn certificates update', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnCertificatesUpdate, hooks.getConfig(), flags, {});
  }
  ...
  it('updates certificate in JSON mode', async () => {
  ...
  it('displays updated certificate in non-JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
  ...
  it('errors when hostname is missing', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'certificate-id': 'cert-1',
    ...
    try {
      ...
      expect(errorStub.firstCall.args[0]).to.include('Hostname is required');
    }
  });
  ...
  it('errors on API failure', async () => {
    ...
    stubCdnRwClient(command, {
      PATCH: async () => ({data: undefined, error: {title: 'Not Found', detail: 'Certificate not found'}}),
    });
