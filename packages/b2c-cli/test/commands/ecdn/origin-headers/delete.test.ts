...
import EcdnOriginHeadersDelete from '../../../../src/commands/ecdn/origin-headers/delete.js';
import {createIsolatedConfigHooks, createTestCommand} from '../../../helpers/test-setup.js';

describe('ecdn origin-headers delete', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnOriginHeadersDelete, hooks.getConfig(), flags, {});
  }
  ...
  it('deletes origin header in JSON mode', async () => {
  ...
  it('deletes header in non-JSON mode', async () => {
    const command: any = await createCommand({'tenant-id': 'zzxy_prd', zone: 'my-zone'});
  ...
  it('errors on API failure', async () => {
    ...
    stubCdnClient(command, {
      DELETE: async () => ({
        data: undefined,
        error: {title: 'Not Found', detail: 'Header not found'},
      }),
    });
