...
import EcdnWafRulesGet from '../../../../../src/commands/ecdn/waf/rules/get.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../../helpers/test-setup.js';

describe('ecdn waf rules get', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnWafRulesGet, hooks.getConfig(), flags, {});
  }
  ...
  it('returns a rule in JSON mode', async () => {
    ...
    expect(result.rule.ruleId).to.equal('rule-1');
    expect(result.rule.groupId).to.equal('grp-1');
    expect(result.rule.action).to.equal('block');
    expect(result.rule.defaultAction).to.equal('monitor');
  });

  it('displays rule details in non-JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'rule-id': 'rule-2',
    ...
    expect(result.rule.ruleId).to.equal('rule-2');
    expect(result.rule.description).to.equal('XSS Detection');
  });
  ...
  it('errors on API failure', async () => {
    ...
    stubCdnClient(command, {
      GET: async () => ({
        data: undefined,
        error: {title: 'Not Found', detail: 'Rule not found'},
      }),
    });
    ...
    try {
      ...
      expect(errorStub.firstCall.args[0]).to.include('Failed to fetch WAF rule');
    }
  });
  ...
  it('errors when no data returned', async () => {
    ...
    try {
      ...
      expect(errorStub.calledOnce).to.equal(true);
      expect(errorStub.firstCall.args[0]).to.include('No WAF rule data returned');
    }
  });
});
