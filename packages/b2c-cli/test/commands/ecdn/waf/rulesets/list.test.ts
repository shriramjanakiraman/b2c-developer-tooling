...
import EcdnWafRulesetsList from '../../../../../src/commands/ecdn/waf/rulesets/list.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../../helpers/test-setup.js';

describe('ecdn waf rulesets list', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnWafRulesetsList, hooks.getConfig(), flags, {});
  }
  ...
  it('returns rulesets in JSON mode', async () => {
      ...
      GET: async () => ({
        data: {
          data: [
            ...
            {rulesetId: 'rs-2', name: 'Custom Rules', enabled: false, action: 'challenge'},
          ],
        },
      }),
    });
    ...
    expect(result.rulesets[0].enabled).to.equal(true);
    expect(result.rulesets[1].name).to.equal('Custom Rules');
  });

  it('displays rulesets in non-JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
    });
  ...
  it('handles empty rulesets list', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
    });
  ...
  it('errors on API failure', async () => {
    ...
    try {
      ...
      expect(errorStub.firstCall.args[0]).to.include('Failed to fetch WAF rulesets');
    }
  });
