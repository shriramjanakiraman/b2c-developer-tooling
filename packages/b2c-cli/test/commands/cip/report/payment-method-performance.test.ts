...
describe('cip report payment-method-performance', () => {
  ...
  async function createCommand(flags: Record<string, unknown>): Promise<any> {
    return createTestCommand(PaymentMethodPerformance, hooks.getConfig(), flags, {});
  }
  ...
  it('returns report description when describe flag is true', async () => {
    ...
    expect(result.name).to.equal('payment-method-performance');
  ...
  it('executes report and returns data in JSON mode', async () => {
    ...
    const mockClient = {
      query: sinon.stub().resolves({
        columns: ['payment_method', 'transactions'],
        rows: [{payment_method: 'credit_card', transactions: 1000}],
        rowCount: 1,
      }),
    };
