...
describe('cip report product-co-purchase-analysis', () => {
  ...
  async function createCommand(flags: Record<string, unknown>): Promise<any> {
    return createTestCommand(ProductCoPurchaseAnalysis, hooks.getConfig(), flags, {});
  }
  ...
  it('returns report description when describe flag is true', async () => {
    ...
    expect(result.name).to.equal('product-co-purchase-analysis');
  ...
  it('executes report and returns data in JSON mode', async () => {
    ...
    const mockClient = {
      query: sinon.stub().resolves({
        ...
        rows: [{product_a: 'PROD-1', product_b: 'PROD-2', frequency: 25}],
        rowCount: 1,
      }),
    };
